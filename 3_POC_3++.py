...
def main():
    ...
    st.markdown("### Upload your demand data and get ML-powered forecasts and AI insights.")

    safety_pct = 0

    # Show company logo at the top of the sidebar
    st.logo("assets/logo.png", icon_image="assets/logo.png", size=("large"))
    st.sidebar.markdown("<span style='font-weight:bold;'>UST Global</span>", unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        try:
            ...
            with st.spinner("Generating forecasts..."):
                ...
                if original_forecast is not None:
                    ...
                    with metrics_col1:
                        st.metric("Total Forecast (Original)", f"{total_original:,.0f}")
                    with metrics_col2:
                        st.metric("Lead Time (Weeks)", f"{lead_time_weeks}")
                    with metrics_col3:
                        st.metric("Total Forecast (Adjusted)", f"{total_adjusted:,.0f}")
        ...
        - **Delivery Optimization**: Adjust for lead time and safety stock.
